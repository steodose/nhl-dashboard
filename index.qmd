...
# Load and stack daily MVP odds snapshots
odds_files <- list.files(
  path = "data",
  pattern = "^mvp_odds_\\d{4}_\\d{2}_\\d{2}\\.csv$",
  full.names = TRUE
)

stopifnot(length(odds_files) > 0)

mvp_odds_ts <- odds_files |>
  set_names() |>
  purrr::map_dfr(
    ~ read_csv(.x, show_col_types = FALSE) |>
      mutate(
        snapshot_date = stringr::str_extract(basename(.x), "\\d{4}_\\d{2}_\\d{2}") |>
          lubridate::ymd()
      ),
    .id = "file"
  ) |>
  ...
  mutate(
    ...
    # Normalize to 0-1 implied probability
    implied_prob = dplyr::if_else(mgm_winPct > 1, mgm_winPct / 100, mgm_winPct)
  ) |>
...
plot_df %>%
    ...
    geom_point(size = 2) +
    geom_label_repel(
        aes(label = paste0(sub(".* ", "", name), " (", implied_prob, ")"), color = name),
        data = function(d) d %>% group_by(name) %>% slice_tail(n = 1),
        fill = "white",
        label.size = 0.20,
        size = 3,
        show.legend = FALSE,
    ...
    theme_custom() +
    labs(
        title = 'NHL Hart Trophy (MVP) Odds Over Time',
        subtitle = glue::glue("Implied win probability from BetMGM odds. Last updated: {format(latest_snapshot, '%B %d, %Y')}."),
        x = "",
